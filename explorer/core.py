...
def get_input_and_output_shapes(model) -> Union[Dict[str, list], None]:
    input_specs = dict()
    output_specs = dict()

    # get onnx model input
        ...
        if tensor_type.HasField("shape"):
            ...
            for d in tensor_type.shape.dim:
                ...
                if d.HasField("dim_value"):
                    ...
                    sh.append("?")
            input_specs[input.name] = (sh, elem_type_to_name(tensor_type.elem_type))
        else:
...
def summary(model_proto, model_path, model_name, verbose=False):
    if isinstance(model_proto, ModelProto):
        # 1. display model attribute information
        ...
        for opi in model_proto.opset_import:
            ...
            opi_vd += ", "
        summary_str = (
            f"IR Version: {model_proto.ir_version}\n"
        ...
        console = Console()
        p = Panel(summary_str, title=f"{model_name} Summary", highlight=True)
        console.print(p, width=90)

        # 2. display model input and output information
        inp_specs, oup_specs = get_input_and_output_shapes(model_proto)
        ...
        if verbose:
            ...
            for k, v in all_ops.items():
                ...
                a_nodes += v
            print(Fore.BLUE + Style.BRIGHT + "\nSimple Table: " + Style.RESET_ALL)
            print(tabulate(stat_data, headers=["Op Name", "Number"]))
