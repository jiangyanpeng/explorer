...
class Lexer:
    def __init__(self, pattern):
        ...
        for line in lines:
            ...
            assert (
                len(names_and_arguments) == 1
            ), f"Unexpected line: {line}. The valid symbol is: name(input_argument, output_argument)"
    ...
    def parse_list(self):
        ...
        assert (
            self.token == "]"
        ), f"Unexpected end token for list: ], pos: {self.itoken}"
        return lists
