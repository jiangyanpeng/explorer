...
class Model:
    def __init__(self, m: [str, onnx.ModelProto, pathlib.Path], verbose=False, constant_folding: bool = True,
        ...
        self.valid = True
        self.mproto = m
        self.graph = Graph(m, model_name=self.modelname, verbose=verbose,
                           constant_folding=constant_folding, noderename=noderename)

    def save_model(self, f: str, shape_only: bool = False, no_shape: bool = False):
        self.graph.save_model(f, shape_only=shape_only,
                              rawmodel=self.mproto, no_shape=no_shape)
