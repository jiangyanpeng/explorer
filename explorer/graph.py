...
from .core import get_all_used_op_types, get_input_and_output_shapes
...
def contains_shape_tensor(n):
    nodeset = SHAPE_TENSORS.keys()
    shape_tensors = []
...
class Graph:
    ...
    def __findSHAPE_TENSORS__(self):
        self.shape_tensors = []
        for n in self.nodemap.keys():
            shape_tensors = contains_shape_tensor(self.nodemap[n])
    ...
    #         name = name_prefix + '_' + \
    #             str(i) if name_prefix is not None else nodes[0]
    ...
    def value_infer(self, inputs: {}):
        ...
        for output in self.output:
            outputs.append(self.tensormap[output].np)
        return outputs
